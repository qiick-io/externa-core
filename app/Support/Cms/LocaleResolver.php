...
class LocaleResolver
{
    ...
    /**
     * Resolve the active locale for CMS content from query, header, or config.
     */
    ...
    public function fallbackChain(string $preferred): array
    {
        $allowed = array_values(array_filter(
            config('cms.fallback_locales', []),
            fn (mixed $l): bool => is_string($l) && $this->isAllowed($l)
        ));
    ...
    public function allowedLocales(): array
    {
        ...
        return is_array($locales) ? array_values(array_filter($locales, fn ($l) => is_string($l))) : [];
    }
